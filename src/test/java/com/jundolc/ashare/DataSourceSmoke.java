...
import static com.jundolc.ashare.Model.Quote;

/** Optional live, read-only connectivity check; not run by run-tests.sh. */
public final class DataSourceSmoke {
    public static void main(String[] args) throws Exception {
        MarketClient client=new MarketClient(1);
        List<Quote> quotes = args.length>0&&args[0].equals("tencent") ? client.tencentSpot() : client.spot();
