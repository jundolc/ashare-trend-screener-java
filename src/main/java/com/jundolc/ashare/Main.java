...
public final class Main {
    ...
    static int run(String[] args)throws Exception{
        Path config=Path.of(args.length>1&&args[0].equals("--config")?args[1]:"config.properties");Properties p=new Properties();try(Reader r=Files.newBufferedReader(config,StandardCharsets.UTF_8)){p.load(r);}
        ...
        List<Quote> afterBoard=raw.stream().filter(q->prefixes.stream().noneMatch(q.code()::startsWith)).toList();int st=excludeSt?(int)afterBoard.stream().filter(q->q.name().toUpperCase().contains("ST")).count():0;
        List<Quote> valid=afterBoard.stream().filter(q->!excludeSt||!q.name().toUpperCase().contains("ST")).filter(q->q.price()>0&&q.open()>0&&q.previousClose()>0&&q.volumeShares()>=0).toList();int invalid=afterBoard.size()-st-valid.size();
        List<Quote> candidates=valid.stream().filter(q->q.price()<q.previousClose()&&q.price()<q.open()).toList();
        ...
        Rules rules=new Rules(d(p,"volume.ratio.max",.5),b(p,"trend.ma.order",true),i(p,"trend.ma5.rising.days",2));Screener screener=new Screener();List<Match> matches=Collections.synchronizedList(new ArrayList<>());List<String[]> errors=Collections.synchronizedList(new ArrayList<>());AtomicInteger done=new AtomicInteger();
        ...
        Path out=Path.of(p.getProperty("output.dir","output"));Files.createDirectories(out);String stamp=today.toString();writeMatches(out.resolve("screen_"+stamp+".csv"),matches);writeErrors(out.resolve("errors_"+stamp+".csv"),errors);
